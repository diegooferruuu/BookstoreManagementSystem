    ...
    public static class AuthSeed
    {
        public static async Task EnsureAuthSeedAsync(CancellationToken ct = default)
        {
            ...
            await using (var findAdmin = new NpgsqlCommand("SELECT id FROM users WHERE email=@e LIMIT 1;", conn))
            {
                ...
                if (existing is null)
                {
                    await using var insertAdmin = new NpgsqlCommand(@"INSERT INTO users(username,email,first_name,last_name,password_hash,is_active) VALUES(@u,@e,'Administrator','',@ph,TRUE) RETURNING id;", conn);
                ...
                else
                {
                    ...
                    await using var updateAdmin = new NpgsqlCommand(@"UPDATE users SET username=@u, first_name='Administrator', last_name='', password_hash=@ph, is_active=TRUE WHERE id=@id;", conn);
            ...
            await using (var ensureAdminRole = new NpgsqlCommand(@"INSERT INTO user_roles(user_id, role_id) SELECT @uid, r.id FROM roles r WHERE r.name='Admin' AND NOT EXISTS (SELECT 1 FROM user_roles ur WHERE ur.user_id=@uid AND ur.role_id=r.id);", conn))
            {
                ensureAdminRole.Parameters.AddWithValue("@uid", adminId);
                await ensureAdminRole.ExecuteNonQueryAsync(ct);
            }

            await using (var assignEmployees = new NpgsqlCommand(@"INSERT INTO user_roles(user_id, role_id) SELECT u.id, r.id FROM users u JOIN roles r ON r.name='Employee' LEFT JOIN user_roles ur ON ur.user_id = u.id AND ur.role_id = r.id LEFT JOIN user_roles ura ON ura.user_id = u.id LEFT JOIN roles ra ON ura.role_id = ra.id AND ra.name = 'Admin' WHERE ur.user_id IS NULL AND (u.email IS DISTINCT FROM 'admin@local') AND ra.id IS NULL AND u.is_active = TRUE;", conn))
            {
